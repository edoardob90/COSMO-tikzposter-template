...
%%
%%
%% This version of `tikzposter-example.tex' has been modified by Edoardo Baldi to fit the posters' style
...
\documentclass[25pt, a0paper, portrait, margin=0mm, innermargin=15mm,
 ...
 % --- NEEDED packages (other than loaded by class) and custom settings --- %
 \input{Setup}

 % Poster details: title and author(s)
 
 \title{COSMO TikZ Poster Template}
 
 \author{Tyler Durden}
 
 %%% INSTITUTE and TITLE GRAPHIC  defined in Setup.tex %%%
 ...
 \begin{document}
     ...
     \maketitle[titlegraphictotitledistance=-9cm]

     \begin{columns}%blocks will be placed into columns
         \column{.55}
         \block[roundedcorners=40]{Creating the document (see changed default options, e.g.\ the shape)}{
 ...
 \innerblock[]{Inner Blocks}{Inner blocks may be created inside of blocks with the command \bs\texttt{innerblock[{\it options}]\{{\it Heading}\}\{{\it Text}\}} }
     ...
     \note[targetoffsetx=24cm, targetoffsety=-13cm,rotate=1,angle=270,radius=8cm,width=.75\textwidth,innersep=.4cm]{
     ...
     \column{.45}
         ...
         \block[titlewidthscale=.8,bodywidthscale=.9,titleoffsety=9.5mm,bodyoffsety=9mm]
         %%% bodyoffset[x/y] will shift the block body according to length given
         {Changing the Poster's Appearance}{
